	  	...
	  	<table    class='table table-generic table-strech' >
		    <thead class="bg-light">
		      <tr>
		        ...
		        <th scope="col" class="th-gris text-left " >SubTotal</th>
		    ...
		    <?php $totalfacs=0; ?>
		     @foreach($facturas as $cuenta)
		        <tr>
		          ...
		          <td class='td-titulo text-left'  >{{ $cuenta->anio }}</td>
		          <td class='td-titulo text-left'  >{{ $cuenta->valor_mes }}</td>
		          <td class='td-titulo text-left'  >{{ $cuenta->total_cargos }}</td>
		           <td class='td-titulo text-left'  >-{{ $cuenta->valor_abono ?? 0 }}</td>
		      
		          <td class='text-left'  >${{number_format($cuenta->valor_mes + $cuenta->total_cargos - $cuenta->valor_abono , 0)}}</td>


		       
		             
		        </tr>
		        <?php $totalfacs+=$cuenta->valor_mes + $cuenta->total_cargos - $cuenta->valor_abono; ?>
		      @endforeach
		    </tbody>
		    <tfooter>
		    	<tr>
		       ...
		       <tr>
		    	...
		    	<th>
		    		     ${{ number_format($facturasel->mora, 0) ?? 0 }}
		    	</th>
		       </tr>
		    </tfooter>
		</table>
